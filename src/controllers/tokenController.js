...
module.exports = {
    async create(request, response) {
        ...
        if (!body || !Object.keys(body).includes('email') || !Object.keys(body).includes('password')) {
            response.status(400).json({ message: 'You must send email and password params' })
        }
        ...
        if (isValidPassword) {
            ...
            const obj = jwt.generateTokens({ user: userData.id, userName: `${userData.firstName} ${userData.lastName}` })
